...
import { useSessionContext } from '../context/SessionContext'; // Import useSessionContext

const CustomerDetails: React.FC = () => {
  const { id } = useParams<{ id: string }>();
  const { customers, transactions, updateCustomer, deleteCustomer } = useStore();
  const { appUser } = useSessionContext(); // Get current logged-in user for permissions
  ...
  const [newDebt, setNewDebt] = useState(customer?.debt.toString() || '');

  useEffect(() => {
    if (customer) {
      setNewDebt(customer.debt.toString());
    }
  }, [customer]);
  ...
  const handleUpdateDebt = async () => {
    if (customer && newDebt !== customer.debt.toString()) {
      await updateCustomer({ ...customer, debt: parseFloat(newDebt) || 0 });
    }
              ...
              <div className="flex items-center gap-2 mt-2 bg-white/5 px-4 py-1.5 rounded-full border border-white/5">
                ...
                {isEditingDebt ? (
                  <input
                    ...
                    className="bg-transparent border-none focus:ring-0 text-base font-bold text-center w-24"
                    autoFocus
                  />
                ) : (
                  <span 
                    className={`text-base font-bold ${customer.debt > 0 ? 'text-red-400' : 'text-green-400'} cursor-pointer`}
                    onClick={() => appUser?.permissions.customers && setIsEditingDebt(true)}
                  >

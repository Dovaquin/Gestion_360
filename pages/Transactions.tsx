...
const Transactions: React.FC = () => {
  ...
  const [filterType, setFilterType] = useState<'ALL' | 'SALE' | 'EXPENSE'>(() => {
  ...
  // Update URL when filter changes (optional, but good for keeping state in sync)
  const handleFilterChange = (newType: 'ALL' | 'SALE' | 'EXPENSE') => {
                ...
                <div className="flex flex-col bg-surface-dark rounded-2xl overflow-hidden border border-white/5">
                    {groupItems.map((t, index) => (
                        <Link 
                            ...
                            key={t.id} 
                            className={`flex items-center p-4 gap-3 active:bg-white/5 transition-colors cursor-pointer ${index !== groupItems.length - 1 ? 'border-b border-white/5' : ''}`}
                        >
                            {/* Icon Container */}
